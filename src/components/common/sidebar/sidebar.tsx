    ...
    <Suspense fallback={<NavProjectsSkeleton />}>
      <Sidebar collapsible="icon" className="font-inter bg-white ">
          ...
          <SidebarGroup>
            <SidebarGroupContent>
              <SidebarMenu>
                {items_2.map((item) => (
                  <SidebarMenuItem key={item.title}>
                    <SidebarMenuButton asChild className=" active:hover:bg-primary-1 active:text-white " >
                      <a href={item.url}>
                        <item.icon />
                        <span>{item.title}</span>
                      </a>
        ...
        <SidebarFooter className=" px-4">
          <SidebarMenu>
            {/* <SidebarMenuItem> */}
              <SidebarMenuItem className="group-data-[collapsible=icon]:hidden w-full h-[160px] p-4 rounded-md bg-primary-1
